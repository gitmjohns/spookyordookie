...
export const metadata = { title: "Horror TV — TerrorMeter" };
...
export default async function TVPage({ searchParams }: PageProps) {
  ...
  const [{ titles, count }, user, watchlistIds] = await Promise.all([
    getTV({ page, limit, sort: params.sort ?? "top-rated", genre: params.genre ?? params.subgenre, decade: params.decade }),
    getCurrentUser(),
    getWatchlistIds(),
  ]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        <h1 className="font-display text-4xl text-ghost">TV Shows</h1>
